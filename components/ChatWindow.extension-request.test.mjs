...
test("resets collapse state when a new extension request arrives", () => {
  // Permission-plugin requests render as an inline card instead of the modal
  // (permissionRequest filters them out of modalExtensionDialog); both paths
  // must key the modal off the live request id.
  assert.match(source, /<ExtensionDialog key=\{modalExtensionDialog.id\}/);
