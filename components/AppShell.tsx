...
export function AppShell() {
  ...
  const handleCwdChange = useCallback((
    ...
    // Re-key the parked draft for this cwd into the fresh composer first,
    // then drop the outgoing composer's draft so it cannot re-merge later.
    rekeyDraft(parkedNewSessionDraftKey(cwd), draftKey);
    if (previousDraftKey && previousDraftKey !== draftKey) clearDraft(previousDraftKey);
    setNewSessionDraftId(draftId);
    discardSessionFolderDraft(activeNewSessionDraftKeyRef.current);
    activeNewSessionDraftKeyRef.current = draftKey;
    registerAutoSessionFolderDraftFor(draftKey, cwd, draftId);
    ...
    // Keep the URL on the current pathname so sub-path deployments and file
    // tabs survive a fresh-composer reset.
    router.replace(typeof window !== "undefined" ? window.location.pathname : "/", { scroll: false });
  }, [activeCwd, activeFileTabId, invalidateWorkspaceRestore, newSessionCwd, registerAutoSessionFolderDraftFor, router, selectedSession, restoreWorkspaceContext]);
  ...
  const handleSelectSession = useCallback((session: SessionInfo, isRestore = false, entryId?: string, blockIndex?: number) => {
    ...
    invalidateWorkspaceRestore();
    // Park the outgoing composer's unsent draft so it survives this switch
    // and can be restored when the same cwd gets a fresh composer again.
    const activeDraftKey = activeNewSessionDraftKeyRef.current;
    ...
    // A parked draft may still be claimed by a session being created right
    // now; discardSessionFolderDraft keeps claimed folder intents alive.
    discardSessionFolderDraft(activeDraftKey);
    activeNewSessionDraftKeyRef.current = null;
  ...
  const handleSessionCreated = useCallback((session: SessionInfo, sourceDraftKey: string) => {
    promoteSessionFolderDraft(sourceDraftKey, session.id);
    setRefreshKey((k) => k + 1);
