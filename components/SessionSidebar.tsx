  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 10px 4px", flexShrink: 0 }}>
        <div style={{ position: "relative", flex: 1, minWidth: 0 }}>
          ...
          <input
            ...
            placeholder={t("sidebar.searchSessionsTree")}
            aria-label={t("sidebar.searchSessionsTree")}
            style={{
              width: "100%",
              height: 28,
      ...
      >
        ...
        {/* Search mode reuses upstream's windowed family list for O(viewport)
            mounting. Rows still read real pin/folder state via SessionItem
            props, so organization actions stay correct. Bulk mode keeps the
            tree rows: its checkboxes need bulk props the flat list lacks. */}
        {!loading && !error && hasSearchQuery && sessionFamilies.length > 0 && (
          <div
            style={{
              position: "relative",
              ...
              return (
                ...
                >
                  <SessionItem
                    ...
                    onRenamed={loadSessions}
                    onDeleted={handleDeletedSessionOrganization}
                    isPinned={pinnedIds.has(family.root.id)}
                    onTogglePinned={() => togglePinned(family.root.id)}
                    folders={sessionOrg.folders}
                    currentFolderId={effectiveFolderBySessionId.get(family.root.id) ?? null}
                    onMoveToFolder={(folderId) => moveSessionToFolder(family.root.id, folderId)}
                    onCreateFolder={createFolder}
                  />
                </div>
              );
