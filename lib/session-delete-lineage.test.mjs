...
test("reparenting also fixes the pi-web:subagent metadata parent", () => {
  const f = fixture();
  const metaLine = JSON.stringify({
    type: "custom",
    customType: "pi-web:subagent",
    data: { version: 1, parentSessionId: "parent", parentSessionPath: f.parentPath, profile: "p", description: "d" },
  });
  writeFileSync(f.childPath, readFileSync(f.childPath, "utf8") + metaLine + "\n");
  try {
    const result = reparentDirectChildSessions(
      [info("child", f.childPath, "parent")],
      "parent",
    ...
    const content = readFileSync(f.childPath, "utf8");
    const meta = JSON.parse(content.trimEnd().split("\n").pop());
    // Header reparent ...
    assert.equal(header(f.childPath).parentSession, f.grandPath);
    // ... and the subagent metadata entry follow the new parent.
    assert.equal(meta.data.parentSessionId, "grand");
    assert.equal(meta.data.parentSessionPath, f.grandPath);
    assert.equal(meta.data.profile, "p");
  } finally { rmSync(f.root, { recursive: true, force: true }); }
});

test("subagent metadata for a different parent session is left untouched", () => {
  const f = fixture();
  const metaLine = JSON.stringify({
    type: "custom",
    customType: "pi-web:subagent",
    data: { version: 1, parentSessionId: "someone-else", parentSessionPath: "C:/other.jsonl" },
  });
  writeFileSync(f.childPath, readFileSync(f.childPath, "utf8") + metaLine + "\n");
  try {
    reparentDirectChildSessions(
      [info("child", f.childPath, "parent")],
      "parent",
      f.parentPath,
      f.grandPath,
    ...
    const meta = JSON.parse(readFileSync(f.childPath, "utf8").trimEnd().split("\n").pop());
    assert.equal(meta.data.parentSessionId, "someone-else");
  } finally { rmSync(f.root, { recursive: true, force: true }); }
});

test("deleting a root parent leaves subagent metadata ids empty rather than stale", () => {
  const f = fixture();
  const metaLine = JSON.stringify({
    type: "custom",
    customType: "pi-web:subagent",
    data: { version: 1, parentSessionId: "parent", parentSessionPath: f.parentPath },
  });
  writeFileSync(f.childPath, readFileSync(f.childPath, "utf8") + metaLine + "\n");
  try {
    reparentDirectChildSessions(
    ...
    const meta = JSON.parse(readFileSync(f.childPath, "utf8").trimEnd().split("\n").pop());
    // No new parent exists: the stale id must not survive.
    assert.equal(meta.data.parentSessionId, undefined);
  } finally { rmSync(f.root, { recursive: true, force: true }); }
});
