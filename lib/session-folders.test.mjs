...
test("normalizes valid and invalid stored organization", async () => {
  ...
  // An assignment whose target folder was filtered out is dropped too
  // (reference integrity), so no dangling id survives normalization.
  assert.deepEqual(
    normalizeSessionOrganization({ ...valid, folders: ["x", { id: "f2", name: "Ok" }] }),
    { ...valid, assignments: {}, collapsedFolders: ["f1"], folders: [{ id: "f2", name: "Ok" }] },
  );
  assert.deepEqual(
