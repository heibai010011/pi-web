...
async function waitForNativeOutput(record) {
  // node-pty 1.2 connects ConPTY asynchronously: spawn() can return pid=0.
  // Require real shell output (not merely a timer delay), and fail on startup
  // exit/timeout so a broken native install is never mistaken for a skip.
  if (record.offset > 0) return;
  await new Promise((resolve, reject) => {
    const timer = setTimeout(() => finish(new Error("Native PTY produced no output within 10s")), 10_000);
    const data = record.pty.onData(() => finish());
    const exit = record.pty.onExit(({ exitCode }) => finish(new Error(`Native PTY exited before output: ${exitCode}`)));
...
async function createLeaseTestManager(t) {
  ...
  const kills = [];
  const exports = {};
  // Advance only the manager's lease clock. Mocking a real ConPTY's global
  // timers also fires its 5s connection timeout before its worker gets a turn,
  // synthesizing a startup failure and rescheduling the lease via onExit.
  t.mock.timers.enable({ apis: ["setTimeout"] });
  runInNewContext(outputText, {
    exports, process: { ...process, once() {} }, setTimeout, clearTimeout, console,
    require(id) {
      if (id === "./paths") return paths;
      if (id === "node-pty") return { spawn: () => ({
...
test("native PTY starts after install and repeated creation reuses the same workspace process", ptySkip, async (t) => {
  const id = createTerminal(process.cwd(), 80, 24);
  t.after(() => killTerminal(id));
  const record = globalThis.__piWebTerminals.get(id);
  await waitForNativeOutput(record);
...
test("connected terminals outlive the grace period; only the last disconnect starts expiry", async (t) => {
  const { createTerminal, killTerminal, subscribeTerminal, hasTerminal, kills } = await createLeaseTestManager(t);
  const id = createTerminal(process.cwd(), 80, 24);
  t.after(() => killTerminal(id));
  ...
  assert.deepEqual(kills, [undefined], "expiry terminates the PTY, not just its registry entry");
});

test("unclaimed creations expire without requiring a browser cleanup request", async (t) => {
  const { createTerminal, killTerminal, hasTerminal, kills } = await createLeaseTestManager(t);
  const id = createTerminal(process.cwd(), 80, 24);
  t.after(() => killTerminal(id));
  t.mock.timers.tick(TERMINAL_RECONNECT_MS - 1);
  assert.ok(hasTerminal(id));
