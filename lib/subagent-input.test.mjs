...
// Windows without Developer Mode / admin denies symlink() with EPERM; the
// outside-cwd path assertions still run, only the symlink half needs the link.
test("rejects files outside cwd, including symlink targets", { skip: process.platform === "win32" && process.env.PI_WEB_TEST_ALLOW_SYMLINK !== "1" }, async () => {
