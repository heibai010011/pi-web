...
// These execute real shell commands through the SDK; skipped where spawning
// child processes is denied by the host sandbox.
test("agent bash removes host variables while preserving SDK and user environment", { skip: process.env.PI_WEB_TEST_NO_SPAWN === "1" }, async () => {
  const original = {
...
test("agent bash reads current shell settings for every execution", { skip: process.env.PI_WEB_TEST_NO_SPAWN === "1" }, async () => {
...
test("direct bash removes host variables and allows explicit project values", { skip: process.env.PI_WEB_TEST_NO_SPAWN === "1" }, async () => {
  ...
  const cases = [
    {
      ...
      expected: { Path: "project-metadata", PATH: `${agentBinDir}${pathDelimiter("linux")}/usr/bin` },
    },
    {
      options: { agentBinDir: "C:\\pi-agent\\bin", baseEnvironment: { Path: "C:\\Windows" }, platform: "win32" },
      expected: { Path: `C:\\pi-agent\\bin${pathDelimiter("win32")}C:\\Windows` },
    },
  ];
