...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(subagentSource, /const childCwd = isolatedWorktree\?\.path \?\? parent\.cwd/);
  assert.match(subagentSource, /SessionManager\.create\(childCwd, undefined, \{ parentSession: parent\.sessionFile \}\)/);
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  ...
  assert.match(deleteRouteSource, /await deleteSessionFamily\(id\)/);
  assert.match(source, /async shutdownForDeletion\(\)/);
