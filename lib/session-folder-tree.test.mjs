...
test("normalize drops assignments pointing at nonexistent folders", () => {
  const org = normalizeSessionOrganization({
    pinned: [],
    folders: [{ id: "real", name: "Real" }],
    assignments: {
      "sess-real": "real",
      "sess-ghost": "no-such-folder",
      "sess-ungrouped": "__pi-web-ungrouped__",
    },
    collapsedFolders: [],
  });
  assert.equal(org.assignments["sess-real"], "real");
  assert.equal(org.assignments["sess-ghost"], undefined);
  assert.equal(org.assignments["sess-ungrouped"], "__pi-web-ungrouped__");
});
