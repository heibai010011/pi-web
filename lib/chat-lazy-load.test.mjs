...
test("chat anchor mode persists and normalizes storage values", async () => {
  const { CHAT_ANCHOR_MODE_STORAGE_KEY, DEFAULT_CHAT_ANCHOR_MODE, normalizeChatAnchorMode } = await loadSubject();

  assert.equal(DEFAULT_CHAT_ANCHOR_MODE, "tail");
  assert.equal(normalizeChatAnchorMode("tail"), "tail");
  assert.equal(normalizeChatAnchorMode("prompt-anchor"), "prompt-anchor");
  assert.equal(normalizeChatAnchorMode("bogus"), null);
  assert.equal(normalizeChatAnchorMode(null), null);
  assert.equal(CHAT_ANCHOR_MODE_STORAGE_KEY, "pi-chat-anchor-mode");
});
